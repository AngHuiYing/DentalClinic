...
// 验证患者权限 - 确保这是患者自己的医疗记录
$auth_stmt = $conn->prepare("
    SELECT COUNT(*) as count 
    FROM medical_records 
    WHERE id = ? AND patient_email = ? AND report_generated = TRUE
");
$auth_stmt->bind_param("is", $record_id, $patient_email);
$auth_stmt->execute();
$auth_result = $auth_stmt->get_result()->fetch_assoc();

if ($auth_result['count'] == 0) {
    die("Medical record not found or report not available.");
}
...
<head>
    <meta charset="UTF-8">
    <title>Medical Report - Dental Clinic</title>
    <style>
        body {
        ...
        .patient-copy-notice h3 {
            color: #2c5aa0;
            margin: 0 0 10px 0;
            border: none;
            padding: 0;
        }
        .patient-copy-notice p {
            color: #2c5aa0;
            margin: 0;
            font-weight: 500;
        }
        @media print {
            .print-btn, .back-btn, .patient-copy-notice { display: none; }
...
<body>
    <div class="report-container">
        <div class="patient-copy-notice">
            <h3>🦷 PATIENT COPY</h3>
            <p>This is your personal medical record. Please keep it for your records.</p>
        </div>
        ...
        <div style="text-align: center; margin-top: 30px;">
            ...
            <a href="patient_history.php" class="back-btn">
